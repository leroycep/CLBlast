...
pub fn build(b: *std.Build) void {
    ...
    clblast.root_module.c_std = .C11;
    clblast.addIncludePath(.{ .path = "include/" });
    clblast.addIncludePath(.{ .path = "src/" });
...
fn addCSample(b: *std.Build, name: []const u8, clblast: *std.Build.Step.Compile, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) *std.Build.Step.Compile {
    const sample_exe = b.addExecutable(.{
        .name = b.fmt("clblast_sample_{s}_c", .{name}),
        .target = target,
        .optimize = optimize,
    });
    sample_exe.root_module.c_std = .C99;
    sample_exe.addCSourceFile(.{
        .file = .{ .path = b.fmt("samples/{s}.c", .{name}) },
    });
